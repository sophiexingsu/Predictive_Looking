...
seg_combined<-read.csv("seg_combined.csv")
frame_rate=59.9 
#sampled_frame=c()
...
seg_prob %>%
  ...
  mutate(Combined = paste(Movie, Condition, sep = "-"))%>%
    ...
    theme_classic() +  # Use a minimal theme
    annotate("text", x = 460, y = 0.4, label = "Online Participants", color = "lightblue", hjust = 1, vjust = 1, size = 5) +
    annotate("text", x = 460, y = 0.5, label = "fMRI Participants", color = "darkblue", hjust = 1, vjust = 1, size = 5)+theme(legend.position = "none")+theme(text = element_text(size = 10),
        plot.background = element_rect(color = "white"),
        aspect.ratio = 1)
