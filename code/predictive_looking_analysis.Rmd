...
library(grDevices)
library(nlme)
data_file_path<-"/Users/sophie/Library/CloudStorage/Box-Box/Predictive_Looking/data"
...
all_movie<-read.csv("all_movie.csv", header = TRUE, sep = ",")
...
```{r}
frame_rate=59.9
seg_des<-read.csv("/Users/sophie/Library/CloudStorage/Box-Box/Predictive_Looking/data/segmentation_probability_sampled.csv")
seg_des %<>% 
...
for(m in unique(combined_model_10_cross_compare$Movie.x)){
  for(i in c("coarse","fine")){
    ...
    ccf_result <- ccf(dev, probability, lag.max = NULL, plot =  TRUE, main = paste0(d," Segmentation Probability", "&", "Prediction Error for',  m,"))
    #plot()
    dev.off()
...
```{r}
setwd(data_file_path)
PE_eye_uncertainty=read.csv("PE_eye_uncertainty_cleaned.csv")%>%
...
PE_eye_SEM=read.csv("PE_eye_SEM_cleaned.csv")%>%
