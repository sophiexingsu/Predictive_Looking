...
library(grDevices)
library(nlme)
data_file_path<-"insert your datafile here"
...
all_movie<-read.csv("your_predictive_looking_data.csv", header = TRUE, sep = ",")
...
```{r}
frame_rate=59.9
seg_des<-read.csv("your_segmentation_data.csv")
seg_des %<>% 
...
ggplot(aes(x=scaled_dev, y = scaled_est_prob,color=Condition))+geom_smooth(method="lm",se=TRUE)+facet_grid( ~ Movie)+labs(x="Scaled Predictive Looking Error", y="Estimated Segmentation Probability")+
  scale_y_continuous(name="Estimated Segmentation Probability",breaks=seq(-1,1,1))
  theme_classic() +  # Use a minimal theme
    annotate("text", x = 3, y = 0.018, label = "coarse", color = "red", hjust = 1, vjust = 1, size = 3) +
    annotate("text", x = 3, y = 0.040, label = "fine", color = "blue", hjust = 1, vjust = 1, size = 3)+theme(legend.position = "none")
...
PE_eye_uncertainty=read.csv("SEM'S PERFORMANCE DATA")%>%
...
PE_eye_SEM=read.csv("SEM'S OTHER PERFORMANCE DATAS")%>%
